...
# Fix DTensor import issue FIRST, before any transformers imports
def patch_pytorch_utils():
    """Early patch for DTensor import issue"""
    try:
        import torch
        
        # Monkey patch torch.distributed.tensor to avoid import error
        if not hasattr(torch.distributed, '_tensor_patched'):
            try:
                import torch.distributed.tensor
                torch.distributed._tensor_patched = True
            except Exception:
                # Create a dummy DTensor class if import fails
                class DTensor:
                    pass
                
                if not hasattr(torch.distributed, 'tensor'):
                    torch.distributed.tensor = type('Module', (), {})()
                torch.distributed.tensor.DTensor = DTensor
                torch.distributed._tensor_patched = True
                
        print("Applied early DTensor patch")
    except Exception as e:
        print(f"Early DTensor patch failed: {e}")
...
# Fix DTensor import issue - apply this early
def monkey_patch_dtensor():
    """Fix DTensor import issue in transformers pytorch_utils"""
    try:
        import sys
        import importlib
        
        # Force reload transformers modules if already imported
        modules_to_reload = [m for m in sys.modules.keys() if m.startswith('transformers.pytorch_utils')]
        ...
        if not hasattr(pytorch_utils, '_original_id_tensor_storage'):
            pytorch_utils._original_id_tensor_storage = pytorch_utils.id_tensor_storage
        
        def patched_id_tensor_storage(tensor):
            """Patched version that handles DTensor import error"""
            try:
                from torch.distributed.tensor import DTensor
                if isinstance(tensor, DTensor):
                    return id(tensor._local_tensor._typed_storage() if hasattr(tensor._local_tensor, '_typed_storage') else tensor._local_tensor.storage())
                else:
                    return id(tensor._typed_storage() if hasattr(tensor, '_typed_storage') else tensor.storage())
            except ImportError:
                # DTensor not available, use fallback
                return id(tensor._typed_storage() if hasattr(tensor, '_typed_storage') else tensor.storage())
            except Exception as e:
                # Any other error, use basic fallback
                return id(tensor.storage())
        
        pytorch_utils.id_tensor_storage = patched_id_tensor_storage
        print("Successfully applied DTensor monkey patch")
        
    except Exception as e:
        print(f"Warning: Could not apply DTensor monkey patch: {e}")

# Apply the patch as early as possible
monkey_patch_dtensor()

# Additional fallback - directly patch the problematic function
def final_dtensor_patch():
    """Final attempt to patch DTensor issue"""
    try:
        import transformers.pytorch_utils
        
        def safe_id_tensor_storage(tensor):
            """Safe version of id_tensor_storage that doesn't require DTensor"""
            try:
                if hasattr(tensor, '_typed_storage'):
                    return id(tensor._typed_storage())
                else:
                    return id(tensor.storage())
            except Exception:
                return id(tensor)
        
        transformers.pytorch_utils.id_tensor_storage = safe_id_tensor_storage
        print("Applied final DTensor patch")
    except Exception as e:
        print(f"Final DTensor patch failed: {e}")
...
class BGESimilarityReward(nn.Module):
    """
    BGE-based similarity reward model that computes cosine similarity between solution and completion embeddings.
    
    This class inherits from nn.Module to integrate properly with the distributed training setup and 
    model sharding used in GRPO training.
    """
    ...
    def __init__(self, model_name: str = "BAAI/bge-base-en-v1.5"):
        ...
        try:
            ...
            raise ImportError("transformers is required for BGE similarity reward. Please install it with `pip install transformers`.")
        
        self.model_name = model_name
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = AutoModel.from_pretrained(model_name)
        
        # Add config attribute for compatibility with existing trainer code
    ...
    def _get_embeddings(self, texts: list[str]) -> torch.Tensor:
        """Get embeddings for a list of texts using BGE model."""
        if not texts:
            return torch.empty(0, self.model.config.hidden_size, device=self.model.device)
    ...
    def forward(self, solution_texts: list[str], completion_texts: list[str]) -> torch.Tensor:
        """
        Compute cosine similarity between solution and completion embeddings.
        
        Args:
            solution_texts: List of solution texts
            completion_texts: List of completion texts
            
        Returns:
            Tensor of cosine similarities with shape (batch_size,)
        """
        if len(solution_texts) != len(completion_texts):
            raise ValueError("Number of solutions and completions must match")
            
        # Get embeddings for solutions and completions
        solution_embeddings = self._get_embeddings(solution_texts)
        completion_embeddings = self._get_embeddings(completion_texts)
        
        # Compute cosine similarity
        similarities = F.cosine_similarity(solution_embeddings, completion_embeddings, dim=1)
        
        return similarities


def bge_similarity_reward_func(completions, solution: list[str], **kwargs) -> list[float]:
    """
    Placeholder function for BGE similarity reward. This should not be called directly
    when using the BGE model approach, as it will be replaced by BGESimilarityReward instance.
    """
    raise NotImplementedError("This function should be replaced by BGESimilarityReward model instance")
...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        ...
        metadata={"help": "List of reward functions. Possible values: 'accuracy', 'format', 'bge_similarity'"},
    )
    ...
    bge_model_name: str = field(
        ...
        metadata={"help": "BGE model name for similarity reward"},
    )
...
reward_funcs_registry = {
    ...
    "bge_similarity": bge_similarity_reward_func,
}
...
def main(script_args, training_args, model_args):
    ...
    if script_args.is_reward_customized_from_vlm_module:
        ...
        for func in script_args.reward_funcs:
            if func == "bge_similarity":
                # Create BGE similarity reward model instance
                reward_funcs.append(BGESimilarityReward(model_name=script_args.bge_model_name))
            else:
    ...
    # Save and push to hub with DTensor fix
    try:
        # Apply final DTensor patch before saving
        def patch_save_pretrained():
            import transformers.pytorch_utils as pytorch_utils
            def safe_id_tensor_storage(tensor):
                try:
                    return id(tensor._typed_storage() if hasattr(tensor, '_typed_storage') else tensor.storage())
        ...
        patch_save_pretrained()
        trainer.save_model(training_args.output_dir)
    except Exception as e:
        print(f"Error saving model with standard method: {e}")
        print("Trying alternative save method...")
        
        # Alternative: save state dict directly
        try:
            import torch
            ...
            torch.save(model_to_save.state_dict(), os.path.join(training_args.output_dir, "pytorch_model.bin"))
            
            # Save config
            if hasattr(model_to_save, 'config'):
                model_to_save.config.save_pretrained(training_args.output_dir)
            ...
            print("Model training completed but saving failed. You may need to save manually.")
